...
PRODUCT STYACK ($49.99/unit):
- BazzAI Swarm: Connect (WhatsApp), Flow (Payments), Doc (Scanning), Lead (Sales).
...
- Pipeline is synced to Google Sheets for tracking.`;
        ...
        await prisma.productConfig.updateMany({
            where: { userId: userId, productType: 'BAZZ_LEAD' },
            data: {
                systemPrompt,
                knowledgeBase,
                isConfigured: true
        ...
        console.log('Successfully pivoted Bazz-Lead to the "Opportunity Architect" strategy using User ID.');
    } catch (e) {
        console.error(e);
    } finally {
