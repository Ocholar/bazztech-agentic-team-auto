        ...
        <>
            ...
            <div className={cn(
                ...
                <nav className="flex-1 space-y-1 p-4 overflow-y-auto">
                    ...
                    <div className="pt-4 pb-2 px-3 text-xs font-bold uppercase tracking-wider"
                        style={{ color: 'var(--color-secondary)' }}>
                        Management
                    </div>

                    <Link href={configHref} onClick={onClose}
                        className={cn(
                            "flex items-center gap-3 rounded-lg px-3 py-2 text-sm transition-all",
                            pathname.startsWith(configHref) ? activeCls : inactiveCls
                        )}
                        style={pathname.startsWith(configHref) ? activeStyle : {}}>
                        <Settings className="h-4 w-4" /> Configure Prompts
                    </Link>

                    <Link href={crmHref} onClick={onClose}
                        className={cn(
                            "flex items-center gap-3 rounded-lg px-3 py-2 text-sm transition-all",
                            pathname.startsWith(crmHref) ? activeCls : inactiveCls
                        )}
                        style={pathname.startsWith(crmHref) ? activeStyle : {}}>
                        <Send className="h-4 w-4" /> CRM Pipeline
                    </Link>
                </nav>
